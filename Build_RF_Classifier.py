	...
	for m in range(len(tree)):
		for k in range(len(nfeat)):
			...
			for j in range(w):	
				...
				#Building Random Forest Model
				e1=np.shape(i2)[0]
				config[u]=pd.DataFrame(['Random_Forest',tree[m],nfeat[k],'gini'],index=['Classifier','n_est','max_feat','criterion'],columns=[''])
				#print("\nRandom Forest Building Configuration:\n%s" % config)
				print('\nBuilding Model...\n')
				class_wt = "balanced"
			
				RF = RandomForestClassifier(n_estimators=tree[m],max_features=nfeat[k], 
				bootstrap=True, oob_score=True,class_weight=class_wt)
